...
3. **Dispatch de implementer** — kies het model naar de zwaarte van #N (globale `CLAUDE.md`,
   **Modelkeuze subagents**): substantieel/kritiek-pad → Opus 4.8 via `subagent_type: opus48`;
   ...
   >   `uv run --with pytest-xdist pytest -n 4 -m 'not zwaar'` (BO-94). Eén keer, ná je
   >   laatste wijziging; tussendoor volstaat de test of de module die je raakt. Twee
   >   implementers in sessie F draaiden de volle suite 2–4 keer zonder tussenliggende
   >   Edit, dat is 1–3 min per keer voor niets.
