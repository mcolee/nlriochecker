...
@register
class PandZonderRiolering(_ExterneCheck):
    """EXT-008: een BAG-pand zonder riolering in de buurt."""

    id = "EXT-008"
    title = "BAG-verblijfsobject zonder riolering binnen X m (dekkingscheck)"
    severity = Severity.WARNING
    dimension = Dimension.COMPLETENESS
    rol = "bag_pand"
    ...
    def objecten(self, context: CheckContext) -> list:
        """De strengen en putten samen; die vormen de rioleringsdekking."""
        return [*_strengen(context), *_putten(context)]

    def run(self, context: CheckContext) -> Iterator[Finding]:
        """Zoekt BAG-panden zonder streng of put binnen de afstand.

        Er zijn panden aangeleverd, geen verblijfsobjecten. De pandgeometrie is
        daarvan een benadering: een pand met vijf verblijfsobjecten telt hier als
        een. Dat staat ook in de melding zelf, zodat het niet uit het rapport kan
        wegvallen.
        """
        laag = self.laag(context)
        if laag is None:
        ...
        boom = STRtree(riolering)
        for index, geometrie in enumerate(laag.geometries):
            rij = laag.attributes[index] if index < len(laag.attributes) else {}
            if any(
                riolering[int(positie)].distance(geometrie) <= afstand
                for positie in boom.query(geometrie.buffer(afstand))
            ):
                continue
            punt = geometrie.representative_point()
            aantal = rij.get("aantal_verblijfsobjecten")
            yield self.finding(
                context,
                f"bag:pand/{rij.get('identificatie') or index}",
                str(rij.get("identificatie") or f"BAG-pand {index}"),
                f"Geen streng of put binnen {afstand:g} m van dit BAG-pand "
                f"({aantal if aantal is not None else 'onbekend aantal'} verblijfsobjecten). "
                "Er zijn panden aangeleverd en geen verblijfsobjecten; de pandgeometrie is "
                "hier een benadering van de verblijfsobjectlocatie.",
                location=(punt.x, punt.y),
                afstand_m=afstand,
                verblijfsobjecten=aantal,
                bron=laag.source.name,
            )
